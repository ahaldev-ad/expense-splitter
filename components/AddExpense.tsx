...
interface AddExpenseProps {
  ...
  onAddExpense: (title: string, amount: number, payerId: string, sharedByIds: string[], type: 'expense', groupId: string) => void;
}

export const AddExpense: React.FC<AddExpenseProps> = ({ members, groups, activeGroupId, onAddExpense }) => {
  const [title, setTitle] = useState('');
  const [amount, setAmount] = useState('');
  const [payerId, setPayerId] = useState<string>('');
  const [sharedByIds, setSharedByIds] = useState<string[]>([]);
  const [targetGroupId, setTargetGroupId] = useState<string>(activeGroupId === 'all' ? 'general' : activeGroupId);
  ...
  const handleSmartFill = async () => {
    ...
    try {
        // Fix: Provide fallback empty string to satisfy TypeScript strict null checks
        ...
        if (data) {
            ...
            if (data.sharedByIds && Array.isArray(data.sharedByIds)) {
                const validIds = data.sharedByIds.filter((id: string) => members.some(m => m.id === id));
